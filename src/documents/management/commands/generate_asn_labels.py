...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            default=14,
            required=False,
            help="Number of rows on a single label sheet "
                 "defaults to 14"
        )
        parser.add_argument(
            "-c", "--columns",
            ...
            default=4,
            required=False,
            help="Number of columns on a single label sheet "
                 "defaults to 4"            
        )
        parser.add_argument(
            "-p", "--pages",
            ...
            default=1,
            required=False,
            help="Number of label sheets generated "
                 "defaults to 1"            
        )
        parser.add_argument(
            "-s", "--sheet_size",
        ...
        parser.add_argument(
            ...
            default=12,
            required=False,
            help="Fontsize used to produce ASNs"
                 "defaults to 12"    
        )   
    ...
    def handle(self, *args, **options):
        ...
        asn_end = options["asn_start"]  + options["pages"] * options["rows"]  * options["columns"]  - 1

        pdf = FPDF(orientation="Portrait", format=options["sheet_size"])
        pdf.set_font('helvetica', size=options["fontsize"])
        pdf.set_text_color(options['color'][0], options['color'][1], options['color'][2])
        pdf.set_margin(0)
